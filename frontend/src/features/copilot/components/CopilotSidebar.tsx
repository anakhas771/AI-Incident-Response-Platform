    ...
    return (
      ...
      >
        <div className="relative shrink-0 border-b border-zinc-800/80 p-3.5">
          ...
          <div className="mb-3 flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-zinc-600">Conversation history</p>
            </div>
            <button
              onClick={onCreateSession}
              className="inline-flex items-center gap-1.5 rounded-md border border-indigo-500/20 bg-indigo-500/10 px-2.5 py-1.5 text-xs font-semibold text-indigo-300 transition-all hover:border-indigo-400/40 hover:bg-indigo-500/15 hover:text-indigo-200"
          ...
          <div className="relative">
            <Search className="absolute left-3 top-2.5 h-3.5 w-3.5 text-indigo-400/50" />
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => onSearchChange(e.target.value)}
              placeholder="Search investigations"
              className="w-full rounded-md border border-zinc-800 bg-zinc-950/70 py-1.5 pl-8 pr-3 text-xs text-zinc-200 placeholder-zinc-600 outline-none transition-all focus:border-indigo-500/40 focus:bg-zinc-950"
          ...
          <div className="mt-3 flex items-center gap-1 border-b border-zinc-800/70">
            {(['all', 'pinned', 'archived'] as const).map((tab) => (
              <button
                ...
                className={`border-b-2 px-2.5 py-1.5 text-[11px] capitalize transition-all ${
                  filter === tab
        ...
        <div
          ...
          tabIndex={0}
          className="min-h-0 flex-1 space-y-0.5 overflow-y-auto overscroll-contain p-2.5 focus:outline-none"
          role="listbox"
        ...
        >
          {isLoading ? (
            ...
            <div className="flex h-48 flex-col items-center justify-center px-4 text-center text-zinc-600">
              <MessageSquare className="mb-2 h-7 w-7 text-indigo-500/20" />
            ...
            sessions.map((session, index) => {
              ...
              return (
                <div
                  ...
                  className={`group relative flex cursor-pointer items-center justify-between rounded-md border px-2.5 py-2 transition-all ${
                    isActive
                      ? 'border-indigo-500/20 bg-indigo-500/10 text-zinc-100 shadow-[inset_2px_0_0_rgba(129,140,248,0.9)]'
                      : isFocused
                    ...
                    {isEditing ? (
                      <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                        <input
                          ...
                          autoFocus
                          className="w-full rounded border border-indigo-500/50 bg-zinc-950 px-2 py-1 text-xs text-zinc-100 focus:outline-none"
                        />
                        <button
                      ...
                      <div className="flex items-center gap-1.5">
                        {session.is_pinned && (
                          <Pin className="h-3 w-3 shrink-0 fill-indigo-400 text-indigo-400" />
                        )}
                        <span className="block truncate text-xs font-medium">
                          {session.title || 'Untitled Investigation'}
                        </span>
                        {session.unread_count ? (
                          <span className="min-w-4 rounded bg-indigo-400 px-1 text-center text-[9px] font-bold text-indigo-950">
                            {session.unread_count}
                          </span>
                        ) : null}
                      </div>
                  ...
                  {!isEditing && (
                    <div className="flex shrink-0 items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100">
                      <button
                        ...
                        className="rounded p-1 text-zinc-600 transition-colors hover:bg-zinc-800 hover:text-indigo-300"
                      ...
                      <button
                        ...
                        className="rounded p-1 text-zinc-600 transition-colors hover:bg-zinc-800 hover:text-amber-300"
