    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-0.5">Enterprise isolation boundaries & workspace configurations</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card hoverEffect={false} className="border-indigo-900/50 bg-surface-elevated">
          <CardHeader className="flex flex-row items-center justify-between pb-3 border-b border-subtle">
            <div>
              <CardTitle className="text-sm font-semibold text-zinc-100">{organization?.name || mockOrganization.name}</CardTitle>
              <CardDescription className="font-mono text-xs text-indigo-400">{organization?.slug || mockOrganization.slug}</CardDescription>
            </div>
          ...
          <div className="p-4 space-y-3 text-xs">
            <p className="text-zinc-300">{organization?.description || mockOrganization.description}</p>
